...
def analysis_txt():
    ...
    # Apply RRC matched filter to pilot part (optional, if transmitter applied RRC to pilots)
    # h = rrc(beta, span, sps)
    # rx_pilot = lfilter(h, 1.0, rx_pilot)
    # rx_pilot /= np.max(np.abs(rx_pilot))  # Normalize
