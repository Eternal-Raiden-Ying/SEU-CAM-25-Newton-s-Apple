...
def qpsk_llrs_from_constellation(constellation, scale=None):
    """
...
def plot_unwrap_phase_fitting(slope, intercept, x_t, truncated_unwrapped_phase, N):
...
def plot_corrected_constellations(symbols, origin_H_f, delta, intercept, pilot, effective_symbols_num, N, pic_idx, n_rows, n_cols, symbol_len, num_symbols):
    fig, axes = plt.subplots(n_rows, n_cols, figsize=(10, 6))
    for i, index in enumerate(pic_idx):
...
def plot_data_constellations(symbols, origin_H_f, delta, intercept, emit_constellations, N, pic_idx, n_rows, n_cols, symbol_len, num_symbols):
    fig, axes = plt.subplots(n_rows, n_cols, figsize=(10, 12))
    for i, index in enumerate(pic_idx):
        corrected_H_f = origin_H_f / np.exp(-1j * 2 * np.pi / N * (delta * ((index + num_symbols) * symbol_len) + intercept) * np.arange(N))
...
def analysis_txt(plot=False):
    ...
    rx = np.load(fr"D:\Pycharm\SEU-CAM-25-Newton-s-Apple\Channel Measurement\record\LDPC\received_signal_chirp_l2_10_24k_fs48k_N4096_cp1024_S8diff_R1-2_Z27_802.11n_A_scrambler7b_5.npy")
    pilot = np.load(fr"D:\Pycharm\SEU-CAM-25-Newton-s-Apple\Channel Measurement\save\pilot\pilot_different_txt819_2.npy")
    ...
    if plot:
        plot_unwrap_phase_fitting(slope, intercept, x_t, truncated_unwrapped_phase, N)
    ...
    for index, H_f in enumerate(H_fs):
        origin_H_f += H_f * np.exp(-1j * 2 * np.pi / N * (delta * index * symbol_len + intercept) * np.arange(N))
    origin_H_f /= len(H_fs)
    if plot:
    ...
    # data_partials = list()
    # while True:
    #     delay = (np.floor(-delta * (num_symbols+data_symbol_idx) * symbol_len).astype(np.int32)) # TODO: why here needs to add 1, need?
    #     if ofdm_start + (num_symbols + data_symbol_idx + 1) * (N + cp_len) + delay > rx.size :
    #         break
    #     data_partial = rx[ofdm_start + (num_symbols + data_symbol_idx) * (N + cp_len)+delay:
    #                    ofdm_start + (num_symbols + data_symbol_idx + 1) * (N + cp_len)+delay]
    #     data_partials.append(data_partial)
    #     data_symbol_idx += 1
    ...
    # symbols = np.array(data_partials)[:,cp_len:]
    received_bits = list()
    ...
    pic_idx = np.linspace(start=0,
                          ...
                          num=pic_num).astype(np.int32)
    if plot:
        plot_data_constellations(symbols, origin_H_f, delta, intercept, emit_constellations, N, pic_idx, n_rows, n_cols, symbol_len, num_symbols)
    ...
    for index, symbol in enumerate(symbols):
        ...
        constellation = get_constellation(symbols=symbol, H_f=corrected_H_f,
                                         approximation=simple_approximate)
        llr_blocks.append(qpsk_llrs_from_constellation(constellation))
    ...
    with open(r"D:\Pycharm\SEU-CAM-25-Newton-s-Apple\Channel Measurement\src\OFDM\result\shakespeare1.txt", 'wb') as file:
