...
TXT_INPUT_PATH = r"D:\Documents\Coding\Python\SEUCAM\Channel Measurement\temp\Jossy origin.tiff"
TIFF_INPUT_PATH = r"D:\Documents\Coding\Python\SEUCAM\Channel Measurement\temp\shakespace_poem_middle(1).txt"
...
def fit_drift_between(H_start, H_end, gap, *, N=N, symbol_len=None, return_phi=False, plot=False):
    if symbol_len is None:
        raise ValueError("symbol_len must be provided")
    phase_shift = H_end / H_start
    x_auto, auto_unwrapped_phase, meta = phase_unwrap_auto(data=phase_shift )
    slope, intercept = fitting_line(x=x_auto, y=auto_unwrapped_phase, filter=True, residual_th=1.5)

    if plot:
        print(meta)
        plot_impulse_response(np.fft.ifft(H_start), fs)
        plot_impulse_response(np.fft.ifft(H_end), fs)
...
def analysis_txt(plot=False, plot_opt=None, ground_truth=True, tx_bits_path=TXT_INPUT_PATH):
    rx = np.load(
        fr"D:\Documents\Coding\Python\SEUCAM\Channel Measurement\temp\received_txt_chirp_l2_10_24k_fs48k_N8192_cp1024_S8diff_R1-2_Z27_802.11n_A_random_middle_0.8_long_head_2.npy")
    pilot = np.load(
    ...
    if len(Hf_comb) > 0:
        for j, pidx in enumerate(pilot_pos):
            ...
            plot_flag=False
            plot_impulse_response(np.fft.ifft(Hf_comb[j]), fs)
            if j in [1,10,16,17,18,19]:
                plot_flag = False
            delta_j, phi_step_j = fit_drift_between(H_ref, Hf_comb[j], gap, N=N, symbol_len=symbol_len, return_phi=True, plot=plot_flag)
