...
TXT_INPUT_PATH = r"C:\Users\Lenovo\Desktop\Cambridge_Summer_project\shakespace(short).txt"
...
def OFDM_modulate_data_with_comb(symbols, N, cp_len, iteration=5, seed=128):
    """
    ...
    res_len = len(symbols) - K*num_symbols
    symbols = np.concatenate([symbols.flatten(), QPSK_mapping(np.random.randint(0,2,(K-res_len)*2))]) if res_len else symbols.flatten()
    num_symbols = len(symbols) // K
...
def main():
    ...
    # sd.wait()
    # np.save(r"D:\Pycharm\SEU-CAM-25-Newton-s-Apple\Channel Measurement\record\whole_signal_819.npy",signal_cut)
