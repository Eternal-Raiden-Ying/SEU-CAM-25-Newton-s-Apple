...
def receiver(rx: np.ndarray, pilot: np.ndarray, cfg):
    ...
    print_opt       = cfg.print_opt
    print_len       = cfg.print_len
    print_pad       = cfg.print_pad

    symbol_len = N + cp_len
    POS_BINS   = np.arange(1, N//2)
    DATA_BINS  = POS_BINS[cfg.data_start:-cfg.data_tail]
    Nd         = DATA_BINS.size
    ...
    delta_interpolator = DeltaInterpolator(fs=fs, method=cfg.interp_mode, smooth=cfg.interp_smooth)
    ...
    if groundtruth:
        ...
        if cfg.type_bit_w:
            ...
            type_bit = get_bits_from_str(cfg.suffix_map[tail])
            assert int(type_bit.size) == cfg.type_bit_w, f"type suffix cannot use {cfg.type_bit_w} bits to express"
        else:
            type_bit = np.array([])
        size_bit = num_to_bits_msb(int(gt_bits_raw.size), bit_num=cfg.size_bit_w)
        if head_bit:
            gt_bits_raw = np.concatenate([type_bit, size_bit, gt_bits_raw])
        if cfg.use_scrambler:
            gt_bits_scr = scramble_bits(gt_bits_raw,
                                        seed=cfg.scrambler_seed,
                                        mode=cfg.scrambler_mode,
                                        bit_width=cfg.scrambler_bitwidth)
    ...
    M_try = min(int(symbols_all_td.shape[0] * cfg.first_try_portion),
                estimate_M_from_filesize(filesize_bytes=code.K * cfg.ldpc_batch // 8,
    ...
    max_pseudo_iter = cfg.max_pseudo_iter
    alpha_pseudo_H = cfg.pseudo_pilot_alpha  # H 融合平滑系数 [0,1]

    iter_pseudo = 0
    circle_flag = True
    pilot_pos = pilot_pos_global.copy() if cfg.use_comb else np.array([])
    ...
    ).reshape(-1,N)[:, DATA_BINS] if pilot_pos_global.size and cfg.use_comb else np.zeros((0, Nd), complex)
    ...
    while circle_flag and (iter_pseudo < max_pseudo_iter):
        ...
        # const_pll = const_zf
        # 噪声/收缩
        sigmas = robust_sigma(const_pll, per_sc=cfg.sig_trk_per_sc)
        ...
        llr_raw, stat = llr_from_constellation(const_mmse, llr_clip=cfg.ldpc_llr_clip)
        ...
        if np.all(block_done[:blk_num_head]) and not head_decoded_done:
            ...
            file_bits = 0
            size_bit = decoded_head_raw[cfg.type_bit_w: cfg.head_bit]
            for b in size_bit:
        ...
        # 下一轮 pilot
        pilot_pos = choose_next_pilots(data_pos=data_pos, edge_expand_k=cfg.edge_expand,
    ...
    if cfg.use_scrambler:
        decoded_bits_raw = scramble_bits(info_blocks.flatten(),
                                         seed=cfg.scrambler_seed,
                                         mode=cfg.scrambler_mode,
                                         bit_width=cfg.scrambler_bitwidth).ravel()
    else:
    ...
    if head_bit:
        file_bits = 0
        type_bit = decoded_bits_raw[:cfg.type_bit_w]
        size_bit = decoded_bits_raw[cfg.type_bit_w: head_bit]
        for b in size_bit:
            file_bits = (file_bits << 1) | int(b)
    ...
    info = {
        ...
        'type_suffix': type_str if cfg.type_bit_w else None
    }
