...
import os
utils_path = r"F:\.vscode\communication_CAM\SEU-CAM-25-Newton-s-Apple-develop\Channel Measurement\ldpc_jossy"
print(os.path.exists(utils_path))
...
# ---------------------- LDPC 包路径 ----------------------
ldpc_path = r"F:\.vscode\communication_CAM\SEU-CAM-25-Newton-s-Apple-develop\Channel Measurement\ldpc_jossy\py"
if ldpc_path not in sys.path:
    sys.path.insert(0, ldpc_path)

try:
    import ldpc  # 导入你自己的 LDPC 包
except Exception as e:
    raise ImportError(
        f"无法导入 ldpc 包：{e}\n"
        f"请检查 ldpc_path 是否指向 ldpc_jossy/py 目录，并且包含 __init__.py。"
    )

# ---------------------- Local utils import ----------------------
# 如果你还需要导入 utils，可以直接用绝对路径或者相对路径
import sys
import importlib.util

utils_file = r"F:\.vscode\communication_CAM\SEU-CAM-25-Newton-s-Apple-develop\Channel Measurement\src\utils\encode.py"

spec = importlib.util.spec_from_file_location("utils", utils_file)
utils = importlib.util.module_from_spec(spec)
spec.loader.exec_module(utils)

scrambler = utils.scrambler
ldpc_encode_bits = utils.ldpc_encode_bits
...
LDPC_RATE = '3/4'           # '1/2','2/3','3/4','5/6'
LDPC_Z = 27                 # for 802.11n usually 27/54/81
LDPC_PTYPE = 'A'            # only used for 802.16 rate 2/3 or 3/4

# ---------------------- I/O paths (adjust if needed) ----------------------
TXT_INPUT_PATH = r"C:\Users\Lenovo\Desktop\Cambridge_Summer_project\shakespace_poem_middle.txt"
TIFF_INPUT_PATH = r"C:\Users\Lenovo\Desktop\Cambridge_Summer_project\Week1_Challenge\week1_output\file03\files\file03.tiff"
PNG_INPUT_PATH = r"F:\.vscode\communication_CAM\Coat_of_Arms_of_the_University_of_Cambridge_compressed.svg.png"
PILOT_SAVE_PATH = r"C:\Users\Lenovo\Desktop\Cambridge_Summer_project\pilot_different_txt820_seed256_1.npy"
...
def OFDM_modulate_data(symbols, N, cp_len, use_ratio=DATA_USE_RATIO, fill_seed=RANDOM_FILL_SEED):
    """
...
def OFDM_modulate_data_with_comb(
    ...
    front_guard_ratio=0.1, back_guard_ratio=0.1, fill_seed=2025
):
    """
...
def main():
    ...
    bits = get_bits_from_txt(TIFF_INPUT_PATH)
    print(f"raw bits: {bits.shape}")
    #加文件头
    pay_load_bit_len = len(bits)
    header_64bits = u64_to_bits_msb(pay_load_bit_len)
    print(f"header bits: {len(header_64bits)}, value={header_64bits}")
    ...
    # -------- LDPC 编码 --------
    ldpc_bits, (K, Ncw) = ldpc_encode_bits(bits_scr,rate=LDPC_RATE)
    ...
    # sd.wait()
    # np.save(r"C:\Users\Lenovo\Desktop\Cambridge_Summer_project\signalwhole_signal_824.npy",signal_cut)
